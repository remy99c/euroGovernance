...
exports.CERTIFICATION_TYPE_METADATA = void 0;
exports.isCertificationEvidenceRuntimeVerified = isCertificationEvidenceRuntimeVerified;
exports.evaluateCertificationCompleteness = evaluateCertificationCompleteness;
exports.evaluateCertificationRiskFlags = evaluateCertificationRiskFlags;
exports.evaluateCertificationReminders = evaluateCertificationReminders;
exports.CERTIFICATION_TYPE_METADATA = {
...
function isCanonicalRuntimeTimestamp(value) {
 ...
 */
function isCertificationEvidenceRuntimeVerified(evidence, certification, asOfDate = new Date()) {
    const verification = evidence.objectVerification;
    if (!isPlainRuntimeObject(verification))
        return false;
    ...
    const reviewDueMillis = evidence.reviewDueDate
        ...
        : null;
    return (Number.isFinite(asOfMillis) &&
        ...
        evidence.storagePath.startsWith(`tenants/${certification.tenantId}/evidence/`) &&
        ...
        CERTIFICATION_EVIDENCE_STORAGE_GENERATION_PATTERN.test(String(verification.storageGeneration)) &&
 ...
 */
function evaluateCertificationCompleteness(cert, evidenceDocs, asOfDate = new Date()) {
    ...
    const surveillanceMillis = cert.surveillanceAuditDueDate
        ...
        : null;
    const surveillanceAuditOverdue = surveillanceMillis !== null && surveillanceMillis < nowMillis && cert.status === 'active_valid';
    // Find linked evidence matching cert
    const linkedEvidences = evidenceDocs.filter((e) => cert.linkedEvidenceIds && cert.linkedEvidenceIds.includes(e.id));
    const hasValidCertificateDoc = linkedEvidences.some((e) => isCertificationEvidenceRuntimeVerified(e, cert, asOfDate));
    const gaps = [];
    if (isExpired) {
        gaps.push({
 ...
 */
function evaluateCertificationRiskFlags(certifications, evidenceDocs, asOfDate = new Date()) {
    const flags = [];
    let expiredCount = 0;
    ...
    let overallAssuranceRiskLevel = 'low';
 ...
 */
function evaluateCertificationReminders(certifications, _evidenceDocs = [], options = {}) {

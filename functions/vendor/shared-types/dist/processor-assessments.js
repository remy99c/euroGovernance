...
exports.CANONICAL_ASSESSMENT_TEMPLATES = void 0;
exports.calculateProcessorAssessmentScore = calculateProcessorAssessmentScore;
exports.evaluateProcessorAssessmentRiskFlags = evaluateProcessorAssessmentRiskFlags;
exports.evaluateProcessorAssessmentReminders = evaluateProcessorAssessmentReminders;
exports.generateProcessorAssessmentReportPayload = generateProcessorAssessmentReportPayload;
exports.generateProcessorAssessmentSummaryMatrixPayload = generateProcessorAssessmentSummaryMatrixPayload;
// -----------------------------------------------------------------------------
// CANONICAL MASTER ASSESSMENT TEMPLATES
// -----------------------------------------------------------------------------
exports.CANONICAL_ASSESSMENT_TEMPLATES = [
    {
 ...
 */
function calculateProcessorAssessmentScore(assessment) {
 ...
 */
function evaluateProcessorAssessmentRiskFlags(assessment, asOfDate = new Date()) {
    const flags = [];
    const nowMillis = asOfDate.getTime();
    const dueMillis = new Date(assessment.dueDate).getTime();
    // 1. Overdue Submission Warning
    if (['sent', 'in_progress'].includes(assessment.status) &&
        dueMillis < nowMillis) {
        flags.push({
...
function evaluateProcessorAssessmentReminders(assessments, asOfDate = new Date()) {
    const reminders = [];
    const nowMillis = asOfDate.getTime();
...
function generateProcessorAssessmentReportPayload(assessments, options) {
...
function generateProcessorAssessmentSummaryMatrixPayload(assessments, options) {
    const asOfDate = options.asOfDate || new Date();
    ...
    const matrix = assessments.map((a) => {
        ...
        if (a.status === 'accepted')
            completedCount++;
        if (flags.some((f) => f.ruleCode === 'ASSESSMENT_OVERDUE_SUBMISSION'))
            overdueCount++;
        return {
            vendorName: a.vendorName,
            assessmentType: a.assessmentType,
...
//# sourceMappingURL=processor-assessments.js.map
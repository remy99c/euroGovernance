# UI/UX Audit, Information Architecture Redesign & Persona Onboarding Blueprint

> **Date**: 2026-08-15  
> **Platform Version**: `0.1.0`  
> **Scope**: Next.js 14 Web Application (`apps/web`), Information Architecture, Persona-Based Onboarding, and Component Modernization.
...
While `euroGovernance` possesses an **enterprise-grade, mathematically proven backend** (74 passing test suites, strict multi-tenancy, and immutable audit trails), the current frontend user experience suffers from **severe cognitive overload, visual fragmentation, and lack of guided onboarding**.
...
1. **Flat, Unstructured Navigation (17 Tabs)**:
   - Sidebar displays 17 unrelated buttons in a single vertical list without hierarchy.
   - 6 separate tabs exist for vendor/processor functions alone (`processor_inventory`, `processor_assurance_inventory`, `processor_assessments`, `processor_hub`, `processor_transfers`, `certifications`).
   - 4 separate tabs exist for frameworks and controls (`frameworks`, `coverage_dashboard`, `applicability_review`, `controls`).
2. **Persona Blindness**:
   - An **Auditor** (who only needs read-only evidence inspection) sees the exact same noisy sidebar and mutation buttons as a **Tenant Admin**.
   - A **Contributor** (who only needs to upload an ISO cert) is confronted with complex AI Act risk classification matrices and TIA transfer forms.
3. **Disruptive Browser Native Prompts**:
   - Crucial workflows (e.g. Adopting a framework, creating controls, four-eyes evidence approvals, inviting members, AI classification) rely on standard browser `window.prompt()` and `window.confirm()` popups.
4. **The "Empty Canvas" Syndrome**:
   - Newly provisioned tenants load into an empty overview screen with zero guidance on prerequisite sequence (e.g., "Adopt Framework $\to$ Complete Scoping $\to$ Instantiate Controls $\to$ Register Processors").

---

## 🏛️ 2. Proposed Information Architecture (IA) Redesign

We propose consolidating the 17 flat tabs into **5 logical, collapsible navigation clusters**, with global quick-search and contextual breadcrumbs:
...
To transform the initial user experience from overwhelming to delightful, the platform must display a **Role-Specific Onboarding Flow** on first login.
...
**Primary Goal**: Configure organization profile, establish security baselines, and delegate team responsibilities.
...
**Primary Goal**: Execute dynamic scoping, instantiate unified controls, and initiate vendor due diligence.
...
**Primary Goal**: Establish the Article 30 ROPA inventory, evaluate cross-border transfers (TIAs), and verify DPA execution.
...
**Primary Goal**: Inventory organizational AI systems and determine EU AI Act risk classification.

- **Step 1 (AI System Registration)**: Register internal/external AI models (e.g., Customer Support LLM, Biometric ID, Resume Screener).
...
**Primary Goal**: Read-only inspection of scoped controls, evidence lockers, four-eyes sign-offs, and export generation.
...
To replace the current raw tables and browser popups, the following core UI components should be implemented:

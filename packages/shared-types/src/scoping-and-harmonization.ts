...
export function buildControlCoverageSummary(
  ...
  assurance: ControlCoverageAssuranceContext = {
  ...
  const coverageTrusted =
    assurance.currentArtifactVerified && assurance.assuranceTrusted;
  ...
  for (const reqId of control.requirementIds) {
    ...
    const auditExplanation = countsAsCovered
      ? `[${fwTitle} - ${req.sectionCode}] Current assured coverage: ${(coverageRatio * 100).toFixed(
          0
        )}% via ${mappingType} mapping. ${statutoryRationale}`
      : `[${fwTitle} - ${req.sectionCode}] Mapping strength: ${(mappingCoverageRatio * 100).toFixed(
          0
        )}%; current assured coverage: 0%. ${statutoryRationale} The mapping does not prove current implementation or effectiveness.`;
  ...
  const totalObligationsSatisfied = obligations.filter(
    (obligation) => obligation.countsAsCovered
  ).length;
...
export function computeTenantFrameworkCoverage(params: {
  ...
  const hasAuthoritativeEffectiveAssurance = (
    control: TenantControlInstance | Control
  ): boolean => {
    const candidate = control as Control & {
      currentArtifactVerified?: boolean;
      assuranceTrusted?: boolean;
    };
  ...
  for (const ev of evidence) {
    if (ev && typeof ev === 'object') {
      ...
      const isVerifiedCurrentEvidence =
        ev.status === 'valid' &&
      ...
      if (isVerifiedCurrentEvidence && Array.isArray(ev.controlIds)) {
        for (const cid of ev.controlIds) {
          if (typeof cid === 'string' && cid.length > 0) evidenceControlIds.add(cid);
        }
      }
    }
  }
  ...
  for (const fw of frameworks) {
    ...
    const implementedControlsCount = fwControls.filter(
      hasAuthoritativeEffectiveAssurance
    ).length;
    ...
    for (const req of fwReqs) {
      ...
      const mappedAssuredControlExists = fwControls.some(
        (control) =>
